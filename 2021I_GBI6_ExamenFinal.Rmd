...
```{r wd}
getwd()
setwd("C:/Users/SAHEZ IMPORTACION/OneDrive/Documentos/GBI6_ExamenFinal/GBI6_ExamenFinal")
```

### [0.5 PUNTOS] 1.2. Cargue la data "mRNA_expr.csv"
```{r data}

df <- read.csv("C:/Users/SAHEZ IMPORTACION/OneDrive/Documentos/GBI6_ExamenFinal/GBI6_ExamenFinal/data/mRNA_expr.csv")
print(df)
```
...
resultados <- df_long %>% group_by(gen)
```
...
cancer_mama <- resultados %>% group_by(gen) %>% mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
  select(-row)

descr(cancer_mama) %>% tab_df(title = "Tabla 1. Cáncer de mama y la estadística de su nivel de expresión de los diferentes genes",
...
La tabla 1 muestra una variedad de genes donde se realiza un estudio estadistico y descripitvo de cada gen viendo así que los genes estan asociados estrechamente al cáncer de mama
```
...
En la grafica se observa una nube de puntos de los genes que estan en mayor medida en diferentes tipos de cancer y en donde se observa dos tipos de genes mayormente que son GATA3 y FN1
```
